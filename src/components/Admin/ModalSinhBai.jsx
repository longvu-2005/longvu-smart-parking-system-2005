      ...
      <Form onSubmit={handleSinhTuDongBaiDo}>
        <Modal.Body style={{ backgroundColor: '#f8fafc', color: '#0f172a' }}>
          ...
          <Form.Group className="mb-3">
            ...
            {/* 🛠️ ĐỒNG BỘ VALUE GIÁ TRỊ CHUẨN TIẾNG VIỆT ĐỂ TRÁNH LỆCH PHA VỚI TRANGADMIN VÀ TRANGSECURITY */}
            <Form.Select value={coSo} onChange={(e) => setCoSo(e.target.value)} disabled={sinhLoading}>
              <option value="Cơ sở 1 (HOLA)">HOLA</option>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control type="number" min="1" max="50" value={soLuongCho} onChange={(e) => setSoLuongCho(parseInt(e.target.value) || 1)} disabled={sinhLoading} />
          </Form.Group>
        </Modal.Body>
        <Modal.Footer style={{ backgroundColor: '#f8fafc' }}>
          <Button variant="light" onClick={onHide} disabled={sinhLoading}>Hủy</Button>
          <Button type="submit" variant="dark" className="fw-bold" disabled={sinhLoading}>Bắt Đầu Xây Dựng Chỗ</Button>
        </Modal.Footer>
      </Form>
    </Modal>
  );
